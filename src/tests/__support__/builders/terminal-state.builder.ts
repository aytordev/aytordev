...
import type { NeofetchData, TerminalContent } from "../../../domain/entities/terminal-content";
...
const createBuilder = (state: TerminalState): StateBuilder => ({
  withDimensions: (width, height) => createBuilder({ ...state, dimensions: { width, height } }),

  withAnimation: (config) => createBuilder({ ...state, animation: config }),

  withTimeOfDay: (timeOfDay) => createBuilder({ ...state, timeOfDay }),

  withOwner: (owner) => createBuilder({ ...state, owner: { ...state.owner, ...owner } }),

  withContent: (content) => createBuilder({ ...state, content: { ...state.content, ...content } }),
  ...
  withJourney: (journey) => createBuilder({ ...state, content: { ...state.content, journey } }),
  ...
  withTheme: (themeName) => createBuilder({ ...state, themeName }),

  withTimestamp: (timestamp) => createBuilder({ ...state, timestamp }),

  with: (overrides) => createBuilder({ ...state, ...overrides }),

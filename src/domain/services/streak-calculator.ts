...
export const calculateStreak = (days: readonly ContributionDay[]): StreakInfo => {
  ...
  const { currentStreak, longestStreak, tempStreak, isActive } = sortedDays.reduce<StreakAccumulator>(
    (acc, day, i) => {
      if (day.contributionCount === 0) {
        // Reset tempStreak and update longestStreak if needed
        const newLongest = Math.max(acc.longestStreak, acc.tempStreak);
        const newCurrent = acc.currentStreak === 0 && acc.tempStreak > 0 ? acc.tempStreak : acc.currentStreak;
        return {
          ...acc,
          tempStreak: 0,
      ...
      const newIsActive = acc.isActive || (i === 0 && (day.date === today || day.date === yesterday));
